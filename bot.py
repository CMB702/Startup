    ...
    for attempt in range(retries):
        try:
            ...
            if resp.status_code in (503, 429):
                # 503 = Google's servers temporarily overloaded
                # 429 = free-tier rate limit hit, needs a longer cool-down
                wait = 8 * (attempt + 1) if resp.status_code == 429 else 2 * (attempt + 1)
                logging.warning(f"Gemini {resp.status_code}, waiting {wait}s, retry {attempt+1}/{retries}...")
                time.sleep(wait)
                continue
